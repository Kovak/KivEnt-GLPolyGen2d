...
cdef class VertMesh:
    ...
    def __dealloc__(self):
        ...
        cdef int* gl_indices = self._gl_indices
        if vertices != NULL:
            free(vertices)
            vertices = NULL
        if triangles != NULL:
    ...
    cdef int* get_triangle_indices(self):
        cdef Triangle* triangles = self._triangles
        cdef Triangle tri
        cdef int tri_count = self.tri_count
        cdef int i
        cdef int* gl_indices = <int*>calloc(tri_count*3, sizeof(int))
        cdef int a
    ...
    cdef float* get_vert8_float_array(self):
        cdef Vertex8* vert_data = <Vertex4*>self._vertices
        cdef Vertex8 vert
        cdef int vert_count = self.vert_count
    ...
    cdef float* get_vert12_float_array(self):
        cdef Vertex12* vert_data = <Vertex4*>self._vertices
        cdef Vertex12 vert
        cdef int vert_count = self.vert_count
    ...
    cdef float* get_vert16_float_array(self):
        cdef Vertex16* vert_data = <Vertex4*>self._vertices
        cdef Vertex16 vert
        cdef int vert_count = self.vert_count
